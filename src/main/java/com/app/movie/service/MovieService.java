...
public class MovieService {
    ...
    public ResponseDto create(Movie request) {
        ...
        List<Movie> movies = repository.getByName(request.getNameMovie());
        if(movies.size()>0){
            ...
            response.id= request.getIdMovie();
        }
        return response;
    }
    ...
    public Movie update(Movie movie) {
        ...
        Optional<Movie> currentMovie = repository.findById(movie.getIdMovie());
